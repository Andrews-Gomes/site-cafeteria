...
document.addEventListener('DOMContentLoaded', () => {
    fetch('/user-data', { credentials: 'include' }) // Inclui cookies na requisição
        ...
        .then(data => {
            ...
            // Salvar os valores originais dos campos para comparação
            originalData = {
            ...
            // Desabilitar o botão de salvar inicialmente
            document.getElementById('perfil-form').querySelector('button[type="submit"]').disabled = true;
        })
...
// Função para verificar se algum campo foi modificado
function checkForChanges() {
    ...
    // Habilita ou desabilita o botão de salvar com base nas mudanças
    document.getElementById('perfil-form').querySelector('button[type="submit"]').disabled = !hasChanges;
}

// Adicionar eventos de input nos campos para verificar mudanças
...
document.getElementById('perfil-form').addEventListener('submit', (event) => {
    ...
    fetch('/user-data', {
        ...
        .then(result => {
            ...
            // Limpar mensagens de erro e estilos de erro
            ...
            document.querySelectorAll('.form-group input').forEach(input => {
                input.classList.remove('is-invalid');  // Remover a classe is-invalid
            });
            ...
            if (result.errors) {
                ...
                // Mostra o alerta de sucesso quando a atualização for bem-sucedida
                showSuccessAlert();
                // Recarregar a página após 3 segundos (tempo suficiente para ler o alerta)
                setTimeout(() => {
